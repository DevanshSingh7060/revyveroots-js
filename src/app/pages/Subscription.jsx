          ...
          {plans.map((p, i) => {
            ...
            return (<motion.div key={p.key} initial={{ opacity: 0, y: 20 }} animate={{
              ...
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2.5 mb-6 flex-1">
                {p.features.map((f) => (<li key={f} className="flex items-start gap-3" style={{ fontSize: '14px', lineHeight: 1.5, opacity: 0.9 }}>
        ...
        <div className="grid md:grid-cols-2 gap-8 md:gap-12 pt-8" style={{ borderTop: '1px solid rgba(42,37,32,0.08)' }}>
          <div>
            ...
            <ul className="space-y-3">
              {details.included.map((f, i) => (<li key={i} className="flex items-start gap-4" style={{ fontSize: '14px', color: 'rgba(42,37,32,0.85)', lineHeight: 1.6 }}>

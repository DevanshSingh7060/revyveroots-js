...
export default function Career() {
    ...
    return (<div style={{ background: CREAM }} className="min-h-screen">
      ...
      <section data-tone="light" className="px-5 sm:px-8 lg:px-14 pt-24 pb-32 lg:pb-40" style={{ background: CREAM }}>
        ...
        /* ── Form ── */
        <motion.form key="form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.65, ease }} onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                {/* Full Name */}
                <FieldWrapper>
                  <label style={labelStyle}>Full Name</label>
                  <input required value={form.name} onChange={onChange('name')} placeholder="Your full name" style={inputStyle}/>
                </FieldWrapper>

                {/* Email */}
                <FieldWrapper>
                  <label style={labelStyle}>Email Address</label>
                  <input required type="email" value={form.email} onChange={onChange('email')} placeholder="you@email.com" style={inputStyle}/>
                </FieldWrapper>

                {/* Phone */}
                <FieldWrapper>
                  <label style={labelStyle}>Phone Number</label>
                  <input required type="tel" value={form.phone} onChange={onChange('phone')} placeholder="+91 00000 00000" style={inputStyle}/>
                </FieldWrapper>

                {/* Experience */}
                <FieldWrapper>
                  <label style={labelStyle}>Experience (Years)</label>
                  <input required type="number" min="0" max="50" value={form.experience} onChange={onChange('experience')} placeholder="e.g. 3" style={inputStyle}/>
                </FieldWrapper>
                ...
                <div className="md:col-span-2">
                  <FieldWrapper>
                    ...
                    <div style={{ position: 'relative' }}>
                      <select required value={form.category} onChange={onChange('category')} style={{
                ...inputStyle,
                appearance: 'none',
                ...
                color: form.category ? INK : 'rgba(42,37,32,0.4)',
                        ...
                        {jobCategories.map((cat) => (<option key={cat} value={cat} style={{ color: INK }}>
                            {cat}
                          </option>))}
                      </select>
                      {/* Chevron icon */}
                ...
                <div className="md:col-span-2">
                  ...
                  <div onClick={() => fileRef.current?.click()} onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={onDrop} style={{
                marginTop: '8px',
                ...
                <div className="md:col-span-2">
                  <FieldWrapper>
                    ...
                    <textarea rows={5} value={form.message} onChange={onChange('message')} placeholder="Tell us about yourself…" style={{
                ...inputStyle,
                resize: 'none',
                paddingTop: '16px',
                lineHeight: 1.75,
                ...
                <div className="md:col-span-2 mt-6">
                  <motion.button type="submit" disabled={loading} whileHover={!loading ? { scale: 1.015, y: -2 } : undefined} whileTap={!loading ? { scale: 0.98 } : undefined} transition={{ duration: 0.3, ease }} className="w-full flex items-center justify-center gap-4 tracking-[0.28em] uppercase" style={{
                fontSize: '11px',
                padding: '20px 32px',
                ...
                if (!loading) {
                    ...
                    {loading ? (<>
                        <motion.span animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: 'linear' }} style={{
                    display: 'inline-block',
                    width: '14px',
                    height: '14px',
